...
public class Logger {
    ...
    HashMap<String, Integer> logLastTime;
    public Logger() {
        logLastTime = new HashMap<>();
    }
    ...
    public boolean shouldPrintMessage(int timestamp, String message) {
        if (!logLastTime.containsKey(message))  {
            logLastTime.put(message, timestamp);
            return true;
        }
        if (logLastTime.containsKey(message) && timestamp - logLastTime.get(message) < 10) {
            return false;
        } else {
            logLastTime.put(message, timestamp);
            return true;
